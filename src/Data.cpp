...
bool Data::saveUserData(Member member, string path = MEMBER_DATABASE) {
    ...
    if (empty) {
        ...
        if (member.occupiedRatings.size() == 0) {
            saveFile << "none" << ",";
        } else {
            for (int i = 0; i < member.occupiedRatings.size(); i++) {
                saveFile << member.occupiedRatings[i];
                if (i + 1 == member.occupiedRatings.size()) {
                    break;
                }
                saveFile << ";";
            }
        ...
        if (member.pendingRequests.size() == 0) {
            ...
            for (int i = 0; i < member.pendingRequests.size(); i++) {
                saveFile << member.pendingRequests[i].id;
...
bool Data::saveHouseData(House house, string path = HOUSE_DATABASE) {
    // Check the file is empty
    ifstream file(path);
    if (!file) {
        cerr << "Cannot open file!" << endl;
    ...
    if (empty) {
        ...
        if (house.houseRating.size() == 0) {
            saveFile << "none" << ",";
        } else {
            for (int i = 0; i < house.houseRating.size(); i++) {
                saveFile << house.houseRating[i];
                if (i + 1 == house.houseRating.size()) {
                    break;
                }
                saveFile << ";";
            }
        ...
        // Save the status
        saveFile << ((house.occupationStatus == true) ? "Rented" : "Not Rented") << ",";
        // Save the occupiers
        ...
        if (house.requestsList.size() == 0) {
            saveFile << "none" << ",";
        } else {
            for (int i = 0; i < house.requestsList.size(); i++) {
                saveFile << house.requestsList[i].id;
                if (i + 1 == house.requestsList.size()) {
                    break;
                }
                saveFile << ";";
            }
        ...
        if (house.occupiersComments.size() == 0) {
            saveFile << "none" << endl;
        } else {
            int count = 0;
            for (map<string, string>::iterator i = house.occupiersComments.begin(); i != house.occupiersComments.end(); i++) {
                saveFile << i->first << ": " << i->second;
                count++;
                if (count == house.occupiersComments.size() - 1) {
                    break;
                }
                saveFile << ";";
            }
...
bool Data::LoadUserData(string memberPath = MEMBER_DATABASE) {
    fstream openFile;
    string line;
    ...
    while (getline(openFile, line)) {
        ...
        userList.push_back(Member(id, fullName, userName, password, phoneNumber, myHouse, occupierRatings, creditPoints, pendingRequests, ownerComment));
    }
    // Close the file
    openFile.close();
    return 0;
...
bool Data::LoadHouseData(string housePath = HOUSE_DATABASE) {
    fstream openFile;
