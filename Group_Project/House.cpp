...
House::House(string houseId, string startDate, string endDate, string address, string location, string description, double consumingPoints, double minOccupierRating) : 
                                                                houseID(houseId), startDate(startDate),
                                                                endDate(endDate), address(address),
                                                                description(description), consumingPoints(consumingPoints),
...
House::House(string houseId, string startDate, string endDate, string address,
             ...
             vector<double> houseRatings, bool status, vector<string> occupierComment) : houseID(houseId), startDate(startDate),
                                                                endDate(endDate),
                                                                address(address),
...
void House::showFullHouse() {
    if (this != nullptr) {
        ...
        if (this->occupierComment.empty()) {
            ...
            for (string comment: occupierComment) {
                cout << "\t";
                cout << comment << endl;
            }
        }
