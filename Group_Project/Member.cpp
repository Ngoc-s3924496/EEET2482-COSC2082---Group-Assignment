...
Member::Member(string id, string fullName, string userName, string password, string phoneNumber, std::vector <double> occupierRatings, int creditPoints,vector<string> ownerComments) {
    memberCounter++;
    this->id = std::move(id);
    this->fullName = std::move(fullName);
    this->username = std::move(userName);
...
void Member::showFullInfo() {
    ...
    for (string x: this->ownerComments) {
        cout << "\t";
        for (auto &i : Data::userList) {
            if (i.username == x.substr(0, x.find(": "))) {
                cout << std::setw(10) << x.substr(0, x.find(": ")) << "-" << i.fullName << " comments: " << x.substr(x.find(": ", x.size())) << endl;
            }
        }
    }
    // new line
