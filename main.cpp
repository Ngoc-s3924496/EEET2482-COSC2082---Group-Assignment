...
public:
    void registerMember() {
        std::map<string, string> userApass;
        ...
        userApass[username] = password;
        cout << "Please enter your full name: ";
        std::getline(cin, fullName);
        cout << "Please enter your phone number: ";
        std::getline(cin, phoneNumber);
        cout << "Your first credit point will be 500" << endl;
        creditPoint = 500;
        cout << "Your start ocupied rating will be 0" << endl;
        occupiedRating = 0;
        Member mem{userApass, fullName, phoneNumber, {}, {}, creditPoint, occupiedRating, {}, {}};
    }
};
...
public:
    House(const string &houseId, const string &startDate, const string &endDate, const string &address,
          const string &location, const string &description, const vector<double> &houseRatings, bool status,
          const vector<Member> &occupiers, const vector<Member> &requestList,
          const std::map<string, string> &occupierComment) : houseID(houseId), startDate(startDate), endDate(endDate),
